...
class ProfileEditController extends BaseController {
  final LocationRepository repository = Get.find(tag: (LocationRepository).toString());
  final MainController mainController = Get.find<MainController>();

  TextEditingController nameController = TextEditingController();
  TextEditingController genderController = TextEditingController();
  TextEditingController phoneNumberController = TextEditingController(text: "62");
  ...
  Future<void> checkIfDataExist() async{
    ...
    if(!user.provinceCode.isEmptyOrNull) await loadProvince(user.provinceCode ?? '');
    if(!user.cityCode.isEmptyOrNull) await loadCity(user.provinceCode??'', user.cityCode ?? '');

    nameController.text = user.fullName ?? '';
    addressController.text = user.address ?? '';
    gender = TypeGender.values.singleWhere((element) => element.valueOutput == user.gender);
    genderController.text = TypeGender.values.singleWhere((element) => element.valueOutput == user.gender).description;
  ...
  Future<void> loadCity(String provinceId, String id)async{
    callDataService(repository.getCity(provinceId, id),
      onSuccess: (city){
  ...
  onPrepareSubmitData() async{
    ...
    if(formKey.currentState!.validate()){
      ...
      if(phoneNumberController.text.length< 10 || phoneNumberController.text.length > 13){
        showErrorMessage("no handphone antara 10-13 digit");
  ...
  onSubmit()async{
    ...
    callDataService(authRepo.updateProfile(formUpdateProfile), onSuccess: _handleVerification);
  }
